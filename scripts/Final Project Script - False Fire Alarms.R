...
ggplot(data2021, aes(x = INCIDENT_BOROUGH, y = false_alarm)) + geom_bar(stat = 'identity', col = 'red') +
  xlab("Borough") + ylab("Number of false fire alarms") + labs(title = "Number of false fire alarms by borough") + theme(
...
approved_inspections <- approved_inspections %>% rename('ZIPCODE' = 'POSTCODE') %>% select(proportion_approved)
...
xyplot(calPlotData, auto.key = list(columns = 2), main = 'Calibration plot - Logistic Regression', xlab = 'Prediction: False alarms below (0) or above (100) city median',
       ylab = 'Observed: Proportion of false alarms below city median', cex.lab = 3)

# KNN
k <- 5
fit_knn <- knn(train = train %>% select(-ZIPCODE, -false_alarms_above_below_median), test = validate %>% 
...
xyplot(calPlotData, auto.key = list(columns = 2), main = 'Calibration plot - KNN', xlab = 'Prediction: False alarms below (0) or above (100) city median',
       ylab = 'Observed: Proportion of false alarms below city median', cex.lab = 3)
...
xyplot(calPlotData, auto.key = list(columns = 2), main = 'Calibration plot - Naive Bayes', xlab = 'Prediction: False alarms below (0) or above (100) city median',
       ylab = 'Observed: Proportion of false alarms below city median', cex.lab = 3)
...
xyplot(calPlotData, auto.key = list(columns = 2), main = 'Calibration plot - Random Forest', xlab = 'Prediction: False alarms below (0) or above (100) city median',
       ylab = 'Observed: Proportion of false alarms below city median', cex.lab = 3)
...
fit_knn2 <- knn(train = t %>% select(-ZIPCODE), test = test %>% select(-ZIPCODE), 
...
xyplot(calPlotData, auto.key = list(columns = 2), main = 'Calibration plot - Logistic Regression', xlab = 'False fire alarm (no = 0 / yes = 100)',
...
xyplot(calPlotData, auto.key = list(columns = 2), main = 'Calibration plot - Random Forest', xlab = 'False fire alarm (no = 0 / yes = 100)',
       ylab = 'Proportion of true fire alarms')
...
fit_glm2 <- glm(false_alarm ~ incident_weekday + incident_hour + 
                 incident_month + last_inspection_status_vio +
                  days_from_median_cert_expiry_date + days_from_median_last_full_inspection_date +
                  days_from_median_violation_date + proportion_approved, data = rbind(train %>% select(-predictions), validate %>% select(-contains('pred'))), 
...
most_false_alarms_by_alarm_box <- most_false_alarms_by_zipcode %>% arrange(desc(predictions)) %>% top_n(10)
