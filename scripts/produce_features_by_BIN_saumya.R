...
xyplot(calPlotData, auto.key = list(columns = 2), main = 'Calibration plot - KNN', xlab = 'False fire alarm (no = 0 / yes = 100)',
       ylab = 'Proportion of true fire alarms')
...
xyplot(calPlotData, auto.key = list(columns = 2), main = 'Calibration plot - Naive Bayes', xlab = 'False fire alarm (no = 0 / yes = 100)',
       ylab = 'Proportion of true fire alarms')
...
cat('The AUC score on the validate set is', auc_knn2, "\n")
...
train_indices <- sample(nrow(most_false_alarms_by_zipcode), 78039)
validation_indices <- sample(setdiff(1:nrow(most_false_alarms_by_zipcode), train_indices), 39020)
test_indices <- sample(setdiff(1:nrow(most_false_alarms_by_zipcode), c(train_indices, validation_indices)), 39019)
...
# Logistic Regression
weights <- ifelse(train$false_alarm == 0, 1, 151296/4782)
...
# best performance is using Logistic Regression Model: AUC = 0.6020433

# refitting model on Train and Validate using logistic regression
weights <- ifelse((rbind(train, validate %>% select(-contains('pred'))))$false_alarm == 0, 1, 113544/3515)
